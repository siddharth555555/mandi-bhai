...
import {
  ...
  PricedCartLine,
} from '../cart/cart-price-token.service';
...
export class OrdersService {
  ...
  async checkout(userId: string, dto: CheckoutDto) {
    ...
    const livePrices = await this.pricing.priceForMany(
    ...
    for (const item of cartItems) {
      ...
      if (!live?.available) {
        throw new BadRequestException(
          `"${name}" just became unavailable — please review your cart`,
        );
      }
      if (
        live.sellingPrice !== Number(item.snapshotPrice) ||
        live.effectiveMoq !== item.snapshotMoq
      ) {
        throw new BadRequestException(
      ...
      if (item.quantity < live.effectiveMoq) {
        throw new BadRequestException(
          `${name}: minimum order quantity is ${live.effectiveMoq}`,
        );
      }
    }

    const grandTotal = cartItems.reduce(
      (sum, i) => sum + Number(i.snapshotPrice) * i.quantity,
      0,
    );
    ...
    const order = await this.dataSource.transaction(async (manager) => {
      ...
      if (dto.paymentMethod === PaymentMethod.PREPAID) {
        ...
        paymentStatus = charge.success
          ? PaymentStatus.COLLECTED
          : PaymentStatus.FAILED;
        orderPaymentStatus = charge.success
