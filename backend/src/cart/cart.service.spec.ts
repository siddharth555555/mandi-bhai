...
describe('CartService', () => {
      ...
      providers: [
        CartService,
        {
          ...
          useValue: {
            findOne: async ({ where }: any) =>
              cartRows.find(
                (c) => c.retailerProfileId === where.retailerProfileId,
              ) ?? null,
        ...
        },
        {
          ...
          useValue: {
            ...
            create: (v: Partial<CartItem>) =>
              ({ id: `item-${++idCounter}`, ...v }) as CartItem,
        ...
        },
        {
          ...
          useValue: {
            findOne: async () =>
              ({ id: RETAILER_ID, userId: USER_ID }) as RetailerProfile,
          },
        },
        {
          ...
          useValue: {
            findOne: async ({ where }: any) =>
              where.id === PRODUCT_ID ? makeProduct() : null,
          },
        },
        {
  ...
  it('adds a new line and snapshots the live price/MOQ', async () => {
    const view = await service.addItem(USER_ID, {
  ...
  it('removeItem removes the line', async () => {
    const added = await service.addItem(USER_ID, {
      productId: PRODUCT_ID,
      quantity: 12,
    });
  ...
  it('removeItem on an unknown item throws NotFoundException', async () => {
    await expect(service.removeItem(USER_ID, 'nope')).rejects.toThrow(
      NotFoundException,
    );
  });
});
