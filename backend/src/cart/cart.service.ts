...
export class CartService {
  ...
  async validate(userId: string) {
    ...
    if (rows.length === 0) {
      return {
        ...(await this.buildView(cart)),
        confirmedPriceToken: null as string | null,
      };
    }

    const prices = await this.pricing.priceForMany(
  ...
  private async buildView(cart: Cart) {
    ...
    const items = rows.map((row) => {
      ...
      const priceChanged =
        !unavailable && livePricePerUnit !== Number(row.snapshotPrice);
      ...
      return {
        ...
        livePricePerUnit,
        lineTotal:
          Math.round(Number(row.snapshotPrice) * row.quantity * 100) / 100,
        moq: row.snapshotMoq,
        liveMoq,
        priceChanged,
        moqChanged,
