...
type Props = NativeStackScreenProps<WholesalerStackParamList, 'WholesalerOrderDetail'>;

export default function WholesalerOrderDetailScreen({ route, navigation }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setError(null);
      setOrder(await getIncomingOrder(token, orderId));
    } catch (e) {
  ...
  const runAction = async (fn: () => Promise<OrderDetail>) => {
  ...
  const onConfirm = () => runAction(() => confirmOrder(token!, orderId));
  const onPack = () => runAction(() => packOrder(token!, orderId));
  const onReject = () => {
    if (!rejectReason.trim()) return;
    runAction(() => rejectOrder(token!, orderId, rejectReason.trim()));
  };
      ...
      {loading ? (
        ...
        <>
          <ScrollView contentContainerStyle={styles.scroll}>
            <View
              style={[styles.statusBanner, { backgroundColor: ORDER_STATUS_STYLE[order.status].bg }]}
            >
              <Text style={[styles.statusText, { color: ORDER_STATUS_STYLE[order.status].fg }]}>
            ...
            <Text style={styles.plainText}>{order.retailerName ?? 'Retailer'}</Text>
            ...
            {rejecting ? (
              <View style={styles.rejectBox}>
                <Text style={styles.sectionTitle}>Reason for rejecting</Text>
                <TextInput
                  style={styles.input}
          ...
          {order.status === 'placed' ? (
            <View style={[styles.footer, { paddingBottom: insets.bottom + 14 }]}>
              {rejecting ? (
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity
                    style={[styles.dangerButton, !rejectReason.trim() && styles.buttonDisabled]}
                ...
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity style={styles.primaryButton} disabled={busy} onPress={onConfirm}>
                    {busy ? (
                      <ActivityIndicator color="#fff" />
                    ) : (
            ...
            <View style={[styles.footer, { paddingBottom: insets.bottom + 14 }]}>
              <TouchableOpacity style={styles.primaryButton} disabled={busy} onPress={onPack}>
                {busy ? (
                  <ActivityIndicator color="#fff" />
                ) : (
                  <Text style={styles.primaryText}>Mark as packed</Text>
                )}
              </TouchableOpacity>
            </View>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  dangerText: { color: '#fff', fontWeight: '800', fontSize: 14 },
});
